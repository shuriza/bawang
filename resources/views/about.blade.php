...
<html lang="en">
  <head>
    <title>Kandangan Fresh</title>
    <link rel="shortcut icon" href="{{ asset('assets/img/alanxcode.jpg') }}" type="image/x-icon" />
    <meta charset="UTF-8" />
    ...
    <meta property="og:title" content="Kandangan fresh" />
  ...
  <body class="font-poppins">
    ...
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="{{ url('/') }}" class="flex items-center space-x-3 rtl:space-x-reverse">
            <img src="{{ asset('assets/img/logo.png') }}" class="h-8 rounded-full" alt="Logo" />
        ...
        <button data-collapse-toggle="navbar-solid-bg" type="button"
                class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-white rounded-lg md:hidden
                       hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-gray-200"
        ...
        @php
            $active   = 'block py-2 px-3 md:p-0 text-white bg-500 rounded
                         md:text-white-500';
            $inactive = 'block py-2 px-3 md:p-0 text-gray-900 rounded hover:bg-gray-100
                         md:hover:bg-transparent md:text-white md:border-0 md:hover:text-green-500';
        @endphp
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-solid-bg">
            <ul class="flex flex-col font-medium mt-4 rounded-lg bg-gray-50
                ...
                <li>
                    <a href="{{ url('/') }}"
                       class="{{ request()->is('/') ? $active : $inactive }}">
                        Home
                    </a>
                </li>
                <li>
                    <a href="{{ url('/about') }}"
                       class="{{ request()->is('about') ? $active : $inactive }}">
                        About
                    </a>
                </li>
                <li>
                    <a href="{{ url('/contact') }}"
                       class="{{ request()->is('contact') ? $active : $inactive }}">
                        Contact
                    </a>
                </li>
                ...
                @guest
                    ...
                    <li>
                        <a href="{{ route('login') }}"
                           class="{{ request()->is('login') ? $active : $inactive }}">
                            Login
                        </a>
                    </li>
                @endguest
                ...
                @auth
                    ...
                    @if($role === 1)
                        <li>
                            <a href="{{ url('/dashboard') }}"
                               class="{{ request()->is('dashboard') ? $active : $inactive }}">
                                Dashboard
                            </a>
                        </li>
                    @else
                    {{-- Customer --}}
                        <li>
                            <a href="{{ url('/profile_customer/' . Auth::id()) }}"
                               class="{{ request()->is('profile_customer*') ? $active : $inactive }}">
                                Profil
                            </a>
                        </li>
                    @endif
                    ...
                    <li>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit"
                                    class="block py-2 px-3 md:p-0 text-gray-900 rounded hover:bg-gray-100
                                           md:hover:bg-transparent md:text-white md:border-0 md:hover:text-green-500">
                                Logout
                            </button>
                        </form>
    ...
    <main>
      <section class="px-4 py-20 grid grid-cols-1 gap-y-4 md:grid-cols-2 md:gap-x-10 md:px-10 lg:px-40 md:items-center">
        <div class="" data-aos="fade-up">
          <img src="{{ asset('assets/img/' . $admin->about_produk_image) }}" alt="Produk" class="w-full h-[30em] object-cover rounded shadow">
        </div>

        <div class="text-gray-900 space-y-4">
          <h1 class="text-5xl font-medium" data-aos="fade-up">Tentang Produk Kami</h1>
          <hr class="border-1 border-green-500 w-1/5" data-aos="fade-up" />
          <div class="text-sm space-y-4" data-aos="fade-up">
            <p>{!! $admin->about_produk_text !!}</p>
          </div>
        </div>
      ...
      <section class="px-4 py-20 grid grid-cols-1 gap-y-4 md:grid-cols-2 md:gap-x-10 md:px-10 lg:px-40 md:items-center">
        <div class="order-2" data-aos="fade-up">
          <img src="{{ asset('assets/img/' . $admin->about_petani_image) }}" alt="Petani" class="w-full h-[30em] object-cover rounded shadow">
        </div>

        <div class="text-gray-900 space-y-4 order-1">
          <h1 class="text-5xl font-medium" data-aos="fade-up">Pak Anshori</h1>
          <hr class="border-1 border-green-500 w-1/5" data-aos="fade-up" />
          <div class="text-sm space-y-4" data-aos="fade-up">
            <p>{!! $admin->about_petani_text !!}</p>
          </div>
        </div>
      </section>
          ...
          <div class="text-center max-w-7xl px-4 lg:px-8 lg:py-16">
            <div class="mt-8 mb-8 grid grid-cols-1 gap-14 lg:grid-cols-4 lg:grid-rows-1 lg:gap-20">
              <div class="space-y-2" data-aos="zoom-in">
                <h1 class="text-6xl text-green-500 font-semibold counter" data-target="200">10</h1>
                <p class="text-slate-100 uppercase font-semibold whitespace-nowrap">Projects Completed</p>
              </div>
              <div class="space-y-2" data-aos="zoom-in">
                <h1 class="text-6xl text-green-500 font-semibold counter" data-target="1200">100</h1>
                <p class="text-slate-100 uppercase font-semibold whitespace-nowrap">Satisfied Customers</p>
              </div>
              <div class="space-y-2" data-aos="zoom-in">
                <h1 class="text-6xl text-green-500 font-semibold counter" data-target="10">2000</h1>
                <p class="text-slate-100 uppercase font-semibold whitespace-nowrap">Years Of Experience</p>
              </div>
              <div class="space-y-2" data-aos="zoom-in">
                <h1 class="text-6xl text-green-500 font-semibold counter" data-target="40">1</h1>
